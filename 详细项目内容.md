...
| 优先级 | 模块 | 任务 |
|--------|------|------|
| P0 | `services/simulator.py` | 完善 cast 命令参数、税率计算逻辑、增加 approve 步骤 |
| P0 | `services/monitor.py` | 接入真实 RPC 测试、优化轮询性能 |
| P1 | `services/analyzer.py` | 集成 LLM RAG 分析、增加合约源码检查规则 |
| P1 | `domain/models.py` | 根据实际仿真输出调整字段定义 |
| P2 | `core/db.py` | 实现数据持久化逻辑（将 AuditReport 写入数据库） |

### 全栈/前端（开发者 B）

| 优先级 | 模块 | 任务 |
|--------|------|------|
| P0 | `app.py` | 美化 Streamlit UI、添加实时刷新机制 |
| P0 | `core/config.py` | 设置团队共享的 `.env` 配置 |
| P1 | `app.py` | 增加代币详情页、历史记录查询 |
| P1 | `domain/db_models.py` | 实现查询仓库（Repository 模式） |
| P2 | 部署 | Docker 容器化 + 部署方案 |
